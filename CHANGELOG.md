...
- Simplified future stable releases: adding the newest versioned release-notes
  file now selects and publishes that version through the full quality gate,
  with an optional one-click manual retry.
...
* Protocol metadata now includes a cryptographically random per-connection nonce and an authenticated extension transcript.
* When both peers support it, a second signed confirmation flight binds both identities, nonces and complete hello messages, preventing metadata replay and modification.
* Peering URIs can use `?secure=required` to reject legacy peers and prevent opportunistic downgrade on sensitive links.
* Direct TLS peer certificates are now required to use the same Ed25519 identity authenticated by the UQDA protocol.
* CI now performs dependency vulnerability scanning and a handshake parser fuzz smoke test.

### Compatibility

* New nodes remain wire-compatible with protocol 0.5 peers. Connections to legacy nodes automatically use the legacy handshake unless `secure=required` is configured.

## [0.5.14] - 2026-06-19

* Go 1.25 is now required to build UQDA

### Added

* The `GroupPassword` option can be used to form closed overlays on top of UQDA
...
* Go 1.24 is now required to build UQDA

### Added

* Pledge is now used on OpenBSD to drop privileges after startup
* `uqdactl getPeers` can now optionally sort the peers using `sort=uptime` or `sort=cost`
...
* Upgrade dependencies
* `uqdactl` now renders table borders/separators, can be disabled with `-borders=false`

### Fixed

* `getPeers` no longer incorrectly surfaces error states that haven't happened
* UQDA will no longer attempt reconnections after detecting that a peering connection is to itself
...
* Multicast discovery should now work again when building UQDA as an Android framework
...
* Go 1.21 is now required to build UQDA
...
* UQDA will no longer panic at startup when duplicate peers are configured
...
* New routing scheme, which is backwards incompatible with previous versions of UQDA
  * The wire protocol version number, exchanged as part of the peer setup handshake, has been increased to 0.5
  * Nodes running this new version **will not** be able to peer with earlier versions of UQDA
...
* UQDA will no longer request BBR congestion control for TCP and TLS peerings on Linux
...
* `uqdactl` has been refactored and now has cleaner output
...
* UQDA now maintains configuration internally, rather than relying on a shared and potentially mutable structure
...
* UQDA should now shut down gracefully and clean up correctly when running as a Windows service
...
* `uqdactl getSelf` should now report coordinates correctly again

### Changed

* Go 1.20 is now required to build UQDA
...
* IP addresses are now derived from ed25519 public (signing) keys
  ...
  * Importantly, this means that **all internal IPv6 addresses will change with this release** — this will affect anyone running public services or relying on UQDA for remote access
...
* Many of the admin functions available over `uqdactl` have been changed or removed as part of rewrites to the code
...
* Tunnel routing (a.k.a. crypto-key routing or "CKR") has been removed
  ...
  * We recommend tunnelling an existing standard over UQDA instead (e.g. `ip6gre`, `ip6gretap` or other similar encapsulations, using UQDA IPv6 addresses as the tunnel endpoints)
...
* `cmd/uqdasim` has been removed, and is unlikely to return to this repository

## [0.3.16] - 2021-03-18

### Added

* New simulation code under `cmd/uqdasim` (work-in-progress)
...
* Some checks to prevent UQDA-over-UQDA peerings have been added
...
* Default config file for FreeBSD is now at `/usr/local/etc/uqda.conf` instead of `/etc/uqda.conf`
...
* The UQDA API now supports dialing a remote node using the public key instead of the Node ID
...
* The UQDA API now returns public keys instead of node IDs when querying for local and remote addresses
...
* A warning is now produced in the UQDA output at startup when the MTU in the config is invalid or has been adjusted for some reason
...
* Support for TLS listeners and peers has been added, allowing the use of `tls://host:port` in `Peers`, `InterfacePeers` and `Listen` configuration settings - this allows hiding UQDA peerings inside regular TLS connections

### Changed

* Go 1.13 or later is now required for building UQDA
* Some exported API functions have been updated to work with standard Go interfaces:
  * `net.Conn` instead of `uqda.Conn`
  * `net.Dialer` (the interface it would satisfy if it wasn't a concrete type) instead of `uqda.Dialer`
  * `net.Listener` instead of `uqda.Listener`
...
* The core library now includes several unit tests for peering and `uqda.Conn` connections

### Changed

* On recent Linux kernels, UQDA will now set the `tcp_congestion_control` algorithm used for its own TCP sockets to [BBR](https://github.com/google/bbr), which reduces latency under load
* The systemd service configuration in `contrib` (and, by extension, some of our packages) now attempts to load the `tun` module, in case TUN/TAP support is available but not loaded, and it restricts UQDA to the `CAP_NET_ADMIN` capability for managing the TUN/TAP adapter, rather than letting it do whatever the (typically `root`) user can do

### Fixed

* The `uqda.Conn.RemoteAddr()` function no longer blocks, fixing a deadlock when CKR is used while under heavy load

## [0.3.9] - 2019-09-27

### Added

* UQDA will now complain more verbosely when a peer URI is incorrectly formatted
* Soft-shutdown methods have been added, allowing a node to shut down gracefully when terminated
* New multicast interval logic which sends multicast beacons more often when UQDA is first started to increase the chance of finding nearby nodes quickly after startup
...
* A bug in the semantic versioning script which accompanies UQDA for builds has been fixed
...
* UQDA can now send multiple packets from the switch at once, which results in improved throughput with smaller packets or lower MTUs
...
* A deadlock was fixed in the session code which could result in UQDA failing to pass traffic after some time

### Security

* Address verification was not strict enough, which could result in a malicious session sending traffic with unexpected or spoofed source or destination addresses which UQDA could fail to reject
...
* In low-traffic scenarios where there are multiple peerings between a pair of nodes, UQDA now prefers the most active peering instead of the least active, helping to reduce packet reordering
...
* UQDA now has a public API with interfaces such as `uqda.ConnDialer`, `uqda.ConnListener` and `uqda.Conn` for using UQDA as a transport directly within applications
...
* Following a massive refactoring exercise, UQDA's codebase has now been broken out into modules
* Core node functionality in the `uqda` package with a public API
  * This allows UQDA to be integrated directly into other applications and used as a transport
  * IP-specific code has now been moved out of the core `uqda` package, making UQDA effectively protocol-agnostic
* Multicast peer discovery functionality is now in the `multicast` package
* Admin socket functionality is now in the `admin` package and uses the UQDA public API
...
* Multicast interfaces are now re-evaluated more frequently so that UQDA doesn't need to be restarted to use interfaces that have become available since startup
...
* UQDA will now attempt to clean up UNIX admin sockets on startup if left behind by a previous crash
* Admin socket `getTunnelRouting` and `setTunnelRouting` calls for enabling and disabling crypto-key routing during runtime
* On macOS, UQDA will now try to wake up AWDL on start-up when `awdl0` is a configured multicast interface, to keep it awake after system sleep, and to stop waking it when no longer needed
...
* Handling of `getRoutes` etc in `uqdactl` is now working
...
* UQDA now correctly responds to multicast interfaces going up and down during runtime
...
* Support for building UQDA as an iOS or Android framework if the appropriate tools (e.g. `gomobile`/`gobind` + SDKs) are available
* Connection contexts used for TCP connections which allow more exotic socket options to be set, e.g.
  * Reusing the multicast socket to allow multiple running UQDA instances without having to disable multicast
...
* Default `AdminListen` in newly generated config is now `unix:///var/run/uqda.sock`
* Formatting of `getRoutes` in the admin socket has been improved
* Debian package now adds `uqda` group to assist with `AF_UNIX` admin socket permissions
...
* `uqdactl` is now less prone to crashing when parameters are specified incorrectly
...
* Crypto-key routing support for tunnelling both IPv4 and IPv6 over UQDA
...
* Build name and version numbers are now imprinted onto the build, accessible through `uqda -version` and `uqdactl getSelf`
* Add ability to disable admin socket by setting `AdminListen` to `"none"`
* `uqdactl` now tries to look for the default configuration file to find `AdminListen` if `-endpoint` is not specified
* `uqdactl` now returns more useful logging in the event of a fatal error

### Changed

* Switched to Chord DHT (instead of Kademlia, although still compatible at the protocol level)
* The `AdminListen` option and `uqdactl` now default to `unix:///var/run/uqda.sock` on BSDs, macOS and Linux
...
* Reuse the multicast port on supported platforms so that multiple UQDA processes can run
* `uqdactl` now has more useful help text (with `-help` or when no arguments passed)
...
* Make `uqdactl` less case sensitive
