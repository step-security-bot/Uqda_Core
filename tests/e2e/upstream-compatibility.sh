...
TRANSPORT=${UPSTREAM_TRANSPORT:-tcp}
case "$TRANSPORT" in
  tcp|tls|quic) ;;
  *) fail 'unsupported interoperability transport' ;;
esac
...
CFG_A=$(write_node_config a - "$TRANSPORT://10.243.11.2:12111")
CFG_B="$E2E_TMP/upstream.conf"
umask 077
"$UPSTREAM_BIN" -genconf -json | jq --arg listener "$TRANSPORT://0.0.0.0:12111" '
  .AdminListen = "none" |
  .MulticastInterfaces = [] |
  .IfName = "ygg-test" |
  .IfMTU = 1280 |
...
echo "[E2E] PASS: bidirectional overlay traffic over $TRANSPORT with pinned upstream Yggdrasil 0.5.14"
